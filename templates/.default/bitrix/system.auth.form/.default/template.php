...
<div class="auth-form mt-5">
    <form name="system_auth_form<?= $arResult["RND"] ?>" method="post" target="_top"
        ...
        <div class="form-group">
            <input class="form-control" type="text" placeholder="Логин или эл. почта" autocomplete="username"
        ...
        <div class="form-group">
            <input class="form-control" type="password" placeholder="Введите пароль" autocomplete="current-password"
                   name="USER_PASSWORD" maxlength="50"><!-- autocomplete="off" -->
        ...
        <div class="auth-form__helpers text-nowrap text-center mb-2">
            <? if ($arResult["STORE_PASSWORD"] == "Y"): ?>
                <label title="<?= GetMessage("AUTH_REMEMBER_ME") ?>" class="form-check">
                    <input class="remember form-check-input" type="checkbox" name="USER_REMEMBER" value="Y"
                           id="USER_REMEMBER_frm"/>
                    <span class="form-check-label">запомнить меня</span>
                </label>
            <? endif ?>
            <noindex><a class="forgot" href="<?= $arResult["AUTH_FORGOT_PASSWORD_URL"] ?>" data-fancybox
                        data-type="ajax" rel="nofollow"
                        data-src="/local/components/nikolays93/custom.auth/ajax.php?forgot_password=yes">Забыли
                    пароль?</a></noindex>
        </div>
        ...
        <? if ($arResult["NEW_USER_REGISTRATION"] == "Y"): ?>
            <div class="mt-3 text-center"><!-- col-sm-4 offset-sm-4  -->
                <noindex><a href="/auth/?register=yes" data-fancybox data-type="ajax"
                            data-src="/local/components/nikolays93/custom.auth/ajax.php?register=yes" rel="nofollow">Регистрация</a>
                </noindex>
            </div>
        <? endif ?>
    </form>
